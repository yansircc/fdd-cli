...
| 时机 | 触发条件 | 动作 |
|------|----------|------|
| **Interview 后** | 识别出可自动检测的约束 | 询问用户是否预防性添加 Pitfall |
| **实现完成后** | 代码涉及高风险模式 | 主动建议添加 Pitfall |
| **Bug 修复后** | 传统 FDD 流程 | 记录 Pitfall 防复发 |

### 约束类型 → 触发器映射

| CDD 约束 | 适合的 FDD 触发器 | 预防性 Pitfall？ |
|----------|------------------|-----------------|
| BC (行为) | `rule` / `dynamic` | ✅ 推荐 |
| SC (结构) | `rule` (lint) / `protect` | ✅ 推荐 |
| Non-Goals | `protect` / `command` | ✅ 强烈推荐 |
| RC (资源) | `dynamic` (benchmark) | ⚠️ 可选 |

### 示例：预防性 Pitfall

```bash
# CDD 约束: BC-01 禁止 SQL 字符串拼接
# 虽然还没发生 Bug，但这是高风险模式，值得预防

fdd add --json '{
  "title": "SQL 注入防护",
  "trigger": [{"kind": "rule", "pattern": "\\$\\{.*\\}.*SELECT", "scope": ["src/db/**"]}],
  "replay": {"root_cause": "预防性约束：禁止 SQL 字符串拼接"},
...
**Interview 完成后，主动问用户**：
> 这些约束中，以下几条适合预防性添加为 Pitfall，要现在添加吗？
> - BC-01: 禁止 SQL 拼接 → `rule` 触发器
> - SC-02: 禁止跨模块调用 → `lint` 规则
